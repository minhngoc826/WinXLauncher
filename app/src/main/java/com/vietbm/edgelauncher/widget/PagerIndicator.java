...
public final class PagerIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    private Paint _paintNormal = new Paint(1);
    ...
    private float _expandFactor = 1.1f;
    ...
    public PagerIndicator(Context context, AttributeSet attrs) {
        ...
        _paintNormal.setColor(Color.WHITE);
        _paintNormal.setAlpha(125);
        _paintNormal.setAntiAlias(true);
    }

    @Override
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        _dotSize = getHeight() / 2 - 10;
            ...
            case Mode.DOTS:
                ...
                for (int dot = 0; dot < pageCount; dot++) {
                    ...
                    if (dot == _pager.getCurrentItem()) {
                        ...
                        canvas.drawCircle(_dotSize / 2 + _pad + (_dotSize + _pad * 2) * dot, (float) (getHeight() / 2), _expandFactor * _dotSize / 2, _paintNormal);
                        ...
                        // draw normal dot
                        canvas.drawCircle(_dotSize / 2 + _pad + (_dotSize + _pad * 2) * dot, (float) (getHeight() / 2), _dotSize / 2, _paintNormal);
                    }
                }
                break;
            case Mode.LINES:
